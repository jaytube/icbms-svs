layui.config({
  base:'js/'
	...
	$(function(){
        $(".look").click(function(){
            console.log($("a[data-url='kk/alarm.html']").parent())
            for(let i=0;i<5;i++){
                $("a[data-url='kk/alarm.html']").parent().click();
            }
        })
...
function initMenu() {
    ...
    $.getJSON("sys/menu/userMenu?_"+$.now(), function(r){
        ...
        $("#menuTree li").on("click",function(obj){
	...
	methods: {
		getUser: function(){
			...
			/*	vm.myUpcomingCount=r.myUpcomingCount;
                ...
                vm.currentProject = r.currentProject;
                var obj = {
                    loginName:r.user.loginName
                };
                // document.cookie = "loginName="+ r.user.loginName;
		...
		selectProject: function(selectProjectId){
			var data = "currentProjectId=" + selectProjectId;
            $.ajax({
                type: "POST",
                url: "sys/user/setCurrentProject",
                data: data,
                dataType: "json",
                success: function(result){
                    if(result.code == 0){
                        layer.alert('项目切换成功', function(index){
                            location.reload();
                        });
                    } else {
                        layer.alert(result.msg);
